...
public class AudioVolumeEditWindow : EditorWindow
{
    ...
    Dictionary<string, AphidShake> UIAudioGroup;
    Dictionary<string, AphidShake> SceneAudioGroup;
    //序列化对象
    ...
    {
        UIAudioGroup = new Dictionary<string, AphidShake>();
        SceneAudioGroup = new Dictionary<string, AphidShake>();
        ...
        Dictionary<string, AphidShake> audioDic = new Dictionary<string, AphidShake>();
        if (FileInfo.Exists)
        {
        ...
        //UI
        foreach(object music in Enum.GetValues(typeof(StainToll.UIMusic)))
        {
            string name = music.ToString();
            if (name == "None") continue;
            AphidShake model = new AphidShake();
        ...
        // Scene
        foreach (object music in Enum.GetValues(typeof(StainToll.SceneMusic)))
        {
            string name = music.ToString();
            if (name == "None") continue;
            AphidShake model = new AphidShake();
    ...
    public Dictionary<string, AphidShake> LoadJson(string Filepath)
    {
        StreamReader reader = new StreamReader(Filepath);
        string jsonString = reader.ReadToEnd();
        Dictionary<string, AphidShake> audioGroup = JsonMapper.ToObject<Dictionary<string, AphidShake>>(jsonString);
        reader.Dispose();
        reader.Close();
        return audioGroup;
    }
    ...
    {
        Dictionary<string, AphidShake> dic = new Dictionary<string, AphidShake>();
        MergeDictionaries.Stark(dic, UIAudioGroup);
        MergeDictionaries.Stark(dic, SceneAudioGroup);
