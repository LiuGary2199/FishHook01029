...
public class BuildEditScript : MonoBehaviour
{
    ...
    public static void Check(int buildTargetIndex, BuildInfoData info)
    {
        ...
        if (Resources.FindObjectsOfTypeAll<LopColeJar>()[0].SeedMaw != info.BaseUrl)
        {
            isSame = false;
            Debug.Log("域名");
        }
        ...
        if (Resources.FindObjectsOfTypeAll<ADStrange>()[0].MAX_SDK_KEY != info.Applovin_SDK_KEY)
        {
            isSame = false;
            Debug.Log("admanager key");
        }
        if (Resources.FindObjectsOfTypeAll<ADStrange>()[0].MAX_REWARD_ID != info.Applovin_REWARD_ID)
        {
            isSame = false;
            Debug.Log("reward id");
        }
        if (Resources.FindObjectsOfTypeAll<ADStrange>()[0].MAX_INTER_ID != info.Applovin_INTER_ID)
        {
            isSame = false;
            Debug.Log("inter id");
        }
        if (Resources.FindObjectsOfTypeAll<LopColeJar>()[0].RopeKnow != info.GameCode)
        {
            isSame = false;
            Debug.Log("gamecode");
        }
    ...
    public static void SuperBuild(int buildTargetIndex, BuildInfoData info)
    {
        ...
        Resources.FindObjectsOfTypeAll<ADStrange>()[0].MAX_SDK_KEY = info.Applovin_SDK_KEY;
        Resources.FindObjectsOfTypeAll<ADStrange>()[0].MAX_REWARD_ID = info.Applovin_REWARD_ID;
        Resources.FindObjectsOfTypeAll<ADStrange>()[0].MAX_INTER_ID = info.Applovin_INTER_ID;
        ...
        if (!GameObject.Find("MainManager") || !GameObject.Find("MainManager").GetComponent<SqueakRailStrange>())
        {
            if (GameObject.Find("MainManager"))
            {
                GameObject.Find("MainManager").AddComponent<SqueakRailStrange>();
            }
            else
            {
                GameObject mainObj = new GameObject("MainManager");
                mainObj.AddComponent<SqueakRailStrange>();
            }
        }
        GameObject.Find("MainManager").GetComponent<SqueakRailStrange>().IntentID = info.Adjust_APP_ID;

        if (!GameObject.Find("MainManager").GetComponent<DuneMeStrange>())
        {
            GameObject.Find("MainManager").AddComponent<DuneMeStrange>();
        }

#if UNITY_IOS
        GameObject.Find("MainManager").GetComponent<DuneMeStrange>().appid = info.Rate_ID;
#endif
#if UNITY_ANDROID
        GameObject.Find("MainManager").GetComponent<DuneMeStrange>().appid = info.PackageName;
#endif
        ...
        Resources.FindObjectsOfTypeAll<LopColeJar>()[0].RopeKnow = info.GameCode;
        Resources.FindObjectsOfTypeAll<LopColeJar>()[0].SeedMaw = info.BaseUrl;
        Resources.FindObjectsOfTypeAll<LopColeJar>()[0].SeedReuseMaw = info.BaseUrl + CTedium.ReuseMaw;
        Resources.FindObjectsOfTypeAll<LopColeJar>()[0].SeedTediumMaw = info.BaseUrl + CTedium.TediumMaw;
        Resources.FindObjectsOfTypeAll<LopColeJar>()[0].SeedLineMaw = info.BaseUrl + CTedium.LineMaw;
        Resources.FindObjectsOfTypeAll<LopColeJar>()[0].SeedSqueakMaw = info.BaseUrl + CTedium.SqueakMaw;
        EditorSceneManager.SaveScene(EditorSceneManager.GetActiveScene());

#if UNITY_IOS
        string url = info.BaseUrl + CTedium.TediumMaw + info.GameCode + "&channel=" + "AppStore" + "&version=" + buildWindow.Version;
#elif UNITY_ANDROID
        string url = info.BaseUrl + CTedium.TediumMaw + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#else
        string url = info.BaseUrl + CTedium.TediumMaw + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#endif
       ...
       (data) =>
       {
           ...
           //if (rootData.data.apple_pie != "apple")
           {
               ...
               if (GameObject.Find("LopTombStrange"))
               {
                   DestroyImmediate(GameObject.Find("LopTombStrange"));
               }
           }
       },
       () =>
