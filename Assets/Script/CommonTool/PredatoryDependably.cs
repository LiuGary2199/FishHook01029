...
{
    private static readonly Dictionary<int, Queue<GameObject>> s_ExpertOnCookBox= new Dictionary<int, Queue<GameObject>>();
    private static readonly Dictionary<GameObject, Tween> s_ExpertOnCrouchLeastBox= new Dictionary<GameObject, Tween>();

    private static GameObject LawExpertOnCallCook(GameObject fxPrefab, Transform parentTransform)
    {
        if (fxPrefab == null) return null;
        int prefabId = fxPrefab.GetInstanceID();
        if (!s_ExpertOnCookBox.TryGetValue(prefabId, out Queue<GameObject> pool))
        {
            pool = new Queue<GameObject>();
            s_ExpertOnCookBox[prefabId] = pool;
        }
    ...
    private static void CrouchExpertOnAxCook(GameObject fxObj, int prefabId, Transform parentTransform)
    {
        if (fxObj == null) return;
        if (s_ExpertOnCrouchLeastBox.TryGetValue(fxObj, out Tween oldTween) && oldTween != null)
        {
            oldTween.Kill();
        }
        s_ExpertOnCrouchLeastBox.Remove(fxObj);
    ...
    private static void ShuttleAdditionDestroy(GameObject fxObj)
    {
        if (fxObj == null) return;
    ...
    private static float ScotlandOnCheerful(GameObject fxObj)
    {
        ...
        for (int i = 0; i < systems.Length; i++)
        {
            ...
            float Pipeline= main.duration + lifetime;
            if (Pipeline > maxDuration) maxDuration = Pipeline;
        }
        return maxDuration;
    }
    ...
    public static void PryBeat(GameObject PopBarUp, System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void PryJolt(GameObject PopBarDisapper, System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void UIComaPeatWeed(GameObject GoldImage, int a, Transform StartTF, Transform EndTF, System.Action finish)//ui界面收现金
    {
        ComaPeatWeed(GoldImage, a, StartTF.position, EndTF.position, finish);
    }
    public static void SeatPeatWeed(GameObject GoldImage, int a, Transform StartTF, Transform EndTF, System.Action finish)
    {
        UIFallacyPeatWeed(GoldImage, a, StartTF.position, EndTF.position, finish);
    }

    public static void UnjustSerene(double startNum, double endNum, float delay, TextMeshProUGUI text, System.Action finish)
    {
        UnjustSerene(startNum, endNum, delay, text, "", finish);
    }
    public static void UnjustSerene(double startNum, double endNum, float delay, TextMeshProUGUI text, string prefix, System.Action finish)
    {
        if (text == null)
        {
            finish?.Invoke();
        ...
        {
            if (text != null)
                text.text = prefix + SereneLack.BelterAxNss(x);
    ...
    public static void ComaPeatWeed(GameObject GoldImage, int a, Vector2 StartPosition, Vector2 EndPosition, System.Action finish)
    {
        //如果没有就算了
        if (a == 0)
        {
        ...
        for (int i = 0; i < a; i++)
        {
            ...
            s.Append(GoldIcon.transform.DOMove(new Vector3(GoldIcon.transform.position.x + OffsetX, GoldIcon.transform.position.y + OffsetY, GoldIcon.transform.position.z), 0.15f).SetDelay(Delaytime).OnComplete(() =>
            {
                ...
                if (t % 5 == 0)
                {
                    StainJar.LawLaurasia()?.FoodAttain(StainToll.UIMusic.Sound_GoldCoin1);
                }
            }));
            //金币移动到最终位置
    ...
    public static void UIFallacyPeatWeed(GameObject GoldImage, int a, Vector2 StartPosition, Vector2 EndPosition, System.Action finish)
    {
        //如果没有就算了
        if (a == 0)
        {
        ...
        {
            Debug.LogError("UIDiamondMoveBest: RopeStrange.Instance 为空。");
            finish?.Invoke();
            return;
        }

        if (RopeStrange.Instance.PigFallacyKeepCookRadial == null)
        {
            Debug.LogError("UIDiamondMoveBest: 请在 RopeStrange 上配置 flyDiamondItemPoolPrefab。");
            finish?.Invoke();
            return;
        }
        ...
        for (int i = 0; i < a; i++)
        {
            ...
            Delaytime += 0.06f;
            //复制一个图标
            GameObject GoldIcon = RopeStrange.LawPigFallacyKeepCook(flyDiamondParent);
            if (GoldIcon == null)
            {
            ...
            s.Append(GoldIcon.transform.DOScale(new Vector3(1.5f, 1.5f, 1.5f), 0.3f).SetDelay(Delaytime).SetEase(Ease.OutBack).OnComplete(() =>
            {
                ...
                if (t % 5 == 0)
                {
                    StainJar.LawLaurasia()?.FoodAttain(StainToll.UIMusic.Sound_GoldCoin);
                }
            }));
            s.AppendCallback(() =>
            {
                PigKeep fi = GoldIcon != null ? GoldIcon.GetComponent<PigKeep>() : null;
                if (fi != null && fi.TrailEffect != null)
                    fi.TrailEffect.gameObject.SetActive(true);
            });
            ...
            s.AppendCallback(() =>
            {
                PigKeep fi = GoldIcon != null ? GoldIcon.GetComponent<PigKeep>() : null;
                if (fi != null && fi.TrailEffect != null)
                    fi.TrailEffect.gameObject.SetActive(false);
            });
            s.Append(GoldIcon.transform.DOScale(0.1f, 0.18f).SetEase(Ease.OutQuad));
            s.AppendCallback(() =>
            {
            ...
            s.AppendCallback(() =>
            {
                //收尾
                s.Kill();
                RopeStrange.CrouchPigFallacyKeepCook(GoldIcon, flyDiamondParent);
                if (t == a - 1)
                {
                    finish?.Invoke();
                }
    ...
    private const float SoonExactBrookX= 0.8f;
    private const float SoonExactBrookSoDog= 0.35f;
    private const float SoonExactBrookSoThe= 1.1f;
    private const float SoonExactSeafoodCheerful= 0.15f;
    private const float SoonExactPigCheerful= 0.45f;
    ...
    /// <summary>
    /// 击杀鱼掉金币：使用 <see cref="RopeStrange.CashPoolPrefab"/> 对象池，贝塞尔飞到 HUD 金币位。
    /// </summary>
    public static void SoonSeatPeat(GameObject iconTemplate, int count, Transform startTransform, Transform endTransform, System.Action finish)
    {
        SoonPigPeatUsingCook(FishFlyPoolKind.Cash, iconTemplate, count, startTransform, endTransform, finish);
    }

    /// <summary>
    /// 击杀钻石鱼：使用 <see cref="RopeStrange.DiamondPoolPrefab"/> 对象池，轨迹与 <see cref="FishGoldMove"/> 一致，落地粒子可用 TendStand.FX_DiamondCollect（未配置则复用 FX_Collect）。
    /// </summary>
    public static void SoonFallacyPeat(GameObject iconTemplate, int count, Transform startTransform, Transform endTransform, System.Action finish)
    {
        SoonPigPeatUsingCook(FishFlyPoolKind.Diamond, iconTemplate, count, startTransform, endTransform, finish);
    }

    /// <summary>鱼击杀飞货币公共实现：Cash / Diamond 仅池子与落地粒子预制不同；钻石飞条目父节点为 endTransform，金币在 FX_Cash 下。</summary>
    private static void SoonPigPeatUsingCook(FishFlyPoolKind kind, GameObject iconTemplate, int count, Transform startTransform, Transform endTransform, System.Action finish)
    {
        ...
        if (TendStand.Instance == null || TendStand.Instance.FX_Coma == null)
        {
            Debug.LogError("FishFlyMove: TendStand.FX_Cash 不能为空！");
            finish?.Invoke();
            return;
        }

        GameObject fxCollectPrefab = TendStand.Instance.FX_Crustal;
        if (kind == FishFlyPoolKind.Diamond && TendStand.Instance.FX_FallacyCrustal != null)
        {
            fxCollectPrefab = TendStand.Instance.FX_FallacyCrustal;
        }
        ...
        Transform fxCashRoot = TendStand.Instance.FX_Coma.transform;
        ...
        if (RopeStrange.Instance == null)
        {
            Debug.LogError("FishFlyMove: RopeStrange.Instance 为空。");
            finish?.Invoke();
            return;
        }

        if (kind == FishFlyPoolKind.Cash && RopeStrange.Instance.ComaCookRadial == null)
        {
            Debug.LogError("FishFlyMove: 请在 RopeStrange 上配置 cashPoolPrefab。");
            finish?.Invoke();
            return;
        }

        if (kind == FishFlyPoolKind.Diamond && RopeStrange.Instance.FallacyCookRadial == null)
        {
            Debug.LogError("FishFlyMove: 请在 RopeStrange 上配置 diamondPoolPrefab。");
            finish?.Invoke();
            return;
        }
        ...
        for (int i = 0; i < count; i++)
        {
            GameObject item = kind == FishFlyPoolKind.Cash
                ? RopeStrange.LawComaCook(itemParentTransform)
                : RopeStrange.LawFallacyCook(itemParentTransform);
            ...
            PigKeep flyItem = item.GetComponent<PigKeep>();
            if (flyItem == null)
            {
                Debug.LogError("FishFlyMove: PigKeep 组件不存在！");
                if (kind == FishFlyPoolKind.Cash)
                    RopeStrange.CrouchComaCook(item, itemParentTransform);
                else
                    RopeStrange.CrouchFallacyCook(item, itemParentTransform);
                remaining--;
                continue;
            }

            if (flyItem.TrailEffect != null)
                flyItem.TrailEffect.gameObject.SetActive(false);
         ...
         //   Vector3[] aniPath = BezierUtils.GetBeizerList(popPosition, midPos, endPos, 24);
            Vector3[] aniPath = BezierUtils.GetBeizerList(popPosition, midPos, endPos, 6);
            ...
            sequence.AppendCallback(() =>
            {
                ...
                if (flyItem != null && flyItem.TrailEffect != null)
                    flyItem.TrailEffect.gameObject.SetActive(true);
            });
          //  sequence.Append(item.transform.DOPath(aniPath, 0.6f).SetDelay(0.25f).SetEase(Ease.InCubic).SetLookAt(0.001f, Vector3.right));
            ...
            sequence.AppendCallback(() =>
            {
                GameObject fxTargetEffect = LawExpertOnCallCook(fxCollectPrefab, itemParentTransform);
                if (fxTargetEffect != null)
                {
                    fxTargetEffect.transform.SetAsFirstSibling();
                    fxTargetEffect.SetActive(true);
                    ShuttleAdditionDestroy(fxTargetEffect);
                    int fxPrefabId = fxCollectPrefab.GetInstanceID();
                    float fxDuration = ScotlandOnCheerful(fxTargetEffect);
                    Tween returnTween = DOVirtual.DelayedCall(fxDuration + 0.05f, () =>
                    {
                        CrouchExpertOnAxCook(fxTargetEffect, fxPrefabId, itemParentTransform);
                    });
                    if (s_ExpertOnCrouchLeastBox.TryGetValue(fxTargetEffect, out Tween oldTween) && oldTween != null)
                    {
                        oldTween.Kill();
                    }
                    s_ExpertOnCrouchLeastBox[fxTargetEffect] = returnTween;
                }
                ...
                {
                    SeatBreech(iconTemplate.transform, count);
                }
                ...
                if (remaining == 0)
                {
                    ...
                    StainJar.LawLaurasia().FoodKingdom(Lofelt.NiceVibrations.HapticPatterns.PresetType.LightImpact);
                }
            });
    ...
    public static void SeatBreech(Transform targetTransform, int count)
    {
        // 空值校验
    ...
    public static void AutonomousLayout(GameObject obj, float addPosition, System.Action Finish)
    {
    ...
    /// <summary>
    /// 小游戏开门：在 <see cref="TendStand"/> 根下实例化 <paramref name="flyPrefab"/>，从鱼死亡世界坐标飞到<strong>该父节点本地 (0,0,0)</strong>，结束销毁实例。
    /// </summary>
    public static void AmidRopeBriefNeatPigAxObsessEighty(
        ...
        float flyDuration = 0.55f)
    {
        ...
        if (TendStand.Instance == null)
        {
            Debug.LogWarning("[PredatoryDependably] MiniGameIntroIconFly: TendStand 为空。");
            onComplete?.Invoke();
            return;
        }
        ...
        Transform flyParent = TendStand.Instance.transform;
    ...
    public static void SpectacleScaleani(Transform item)
    {
        //item.transform.localScale = Vector3.one;
        item.DOKill();
        DOTween.Kill(item);
