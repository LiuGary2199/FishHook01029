...
{
    public static void Stark<TKey, TValue>(this IDictionary<TKey, TValue> first, IDictionary<TKey, TValue> second)
    {
...
public class LawSolderMint : MonoSnowstorm<LawSolderMint>
{
    // Start is called before the first frame update
    public bool WoadTeapot;
    public bool BeFill;

    static public LawSolderMint instance;
    ...
    /// <returns></returns>
    public Vector3 LawGourdTrack(GameObject gameobj)
    {
    ...
    /// <returns></returns>
    public GameObject LawTrickleRopeRation(Transform SelfTransform, List<Transform> listTemp)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public float TrackAxFaint(Vector2 p1, Vector2 p2)
    {
        Vector2 p;
        p.x = p2.x - p1.x;
        p.y = p2.y - p1.y;
    ...
    /// <returns></returns>
    public float Faint180Ax360(float angle)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public int LawUnityIntersect(Vector2 p1, Vector2 p2)
    {
        float angle = Faint180Ax360(TrackAxFaint(p1, p2));
    ...
    /// <returns></returns>
    public Vector2 LawAccreteTrueSlit(Renderer Renderer)
    {
    ...
    public Vector2 LawAccreteTrueSlit(Image image)
    {
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public Vector2 LawBehindSlit(GameObject obj)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public Vector2 LawTouchMystique()
    {

        Vector3 v = Input.mousePosition;
        float xa = v.x / Screen.width;
        float ya = v.y / Screen.height;
        float screenW = getPreachMetal();
        float screenH = LawPreachAbroad();
        float x = -screenW / 2 + xa * screenW;
        float y = -screenH / 2 + ya * screenH;
        return new Vector2(x, y);
    }

    public int LawGeyserSlope(List<string> weightList)
    {
        float sumWeight = 0;
    ...
    public int[] LawCodifyElectric(int total, int count)
    {
    ...
    public Dictionary<string, object> BabbleAxStonemason(string value)
    {
        if (value.Length < 1)
        {
            return null;
    ...
    public string StonemasonAxBabble(Dictionary<string, object> Info)
    {
    ...
    public int TopMoodClue(string dateStart, DateTime dateEnd)
    {
    ...
    public int MowMoodClue(string dateStart, DateTime dateEnd)
    {
    ...
    public TimeSpan MoodClue(DateTime dateStart, DateTime dateEnd)
    {
    ...
    public Vector3 LawVanUpBode(Canvas canvas, Text text, string strFragment)
    {
        int strFragmentIndex = text.text.IndexOf(strFragment);//-1表示不包含strFragment
        Vector3 stringPos = Vector3.zero;
        if (strFragmentIndex > -1)
        {
            Vector3 firstPos = LawVanUpBode(canvas, text, strFragmentIndex);
            Vector3 lastPos = LawVanUpBode(canvas, text, strFragmentIndex + strFragment.Length);
            stringPos = (firstPos + lastPos) * 0.5f;
        }
        else
        {
            stringPos = LawVanUpBode(canvas, text, strFragmentIndex);
        }
        return stringPos;
    }

    public Vector3 LawVanUpBode(Canvas canvas, Text text, int charIndex)
    {
    ...
    /// <returns></returns>
    public float SalinityBeFore(Vector3 pB, Vector3 pA1, Vector3 pA2)
    {
    ...
    private static byte[] Atom= { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
    /// <summary>
    /// DES加密字符串
    /// </summary>
    ...
    public static string CitizenDES(string encryptString, string key = "abcdefgh")
    {
        try
        {
    ...
    public static string WeekendDES(string decryptString, string key = "13717421")
    {
        byte[] rgbKey = Encoding.UTF8.GetBytes(Application.identifier.Substring(0, 8));
        byte[] rgbIV = Atom;
