...
public class MoodLack
{
    public static long Balance()
    {
        return LawBoulevard(DateTime.Now);
    }

    public static long LawBoulevard(DateTime datetime)
    {
    ...
    /// <returns></returns>
    public static string MoodLinePrepay(DateTime dt, string format)
    {
        return dt.ToString(format);
    }

    public static DateTime ConfineAxMoodLine(long seconds)
    {
        DateTime startTime = TimeZone.CurrentTimeZone.ToLocalTime(new DateTime(1970, 1, 1)); // 当地时区
    ...
    /// <returns></returns>
    public static string ConfinePrepay(long totalTime, string connector = ":")
    {
    ...
    /// <param name="totalTime"></param>
    /// <returns></returns>
    public static string ConfinePrepay2(long totalTime)
    {
        int seconds = Mathf.Max((int)(totalTime % 60), 0);
        int minutes = Mathf.Max((int)(totalTime / 60) % 60, 0);
    ...
    //根据权重和范围获取奖励数据
public static RewardData LawTempleMintBeGeyserSowDegas(List<RewardData> rewardDatas, bool UseRandom = true)
{
    int totalWeight = 0;
    ...
    foreach (RewardData item in rewardDatas)
    {
        ...
        {
            if (HappenLack.HeDaunt() && item.type == RewardType.Cash)
