...
public class OvercrowdJar : MonoBehaviour
{
    /* 字段 */
    private static OvercrowdJar _Laurasia;              //本脚本私有单例实例
    ...
    /// </summary>
    /// <returns></returns>
    public static OvercrowdJar LawLaurasia()
    {
        if (_Laurasia == null)
        {
            _Laurasia = new GameObject("_ResourceMgr").AddComponent<OvercrowdJar>();
        }
        return _Laurasia;
    }
    ...
    /// <param name="isCatch"></param>
    /// <returns></returns>
    public GameObject GrabWhole(string path, bool isCatch)
    {
