...
public class GameUtil
{
    ...
    private static double GetMulti(RewardType type, double cumulative, MultiGroup[] multiGroup)
    {
        foreach (MultiGroup item in multiGroup)
        {
            if (item.max > cumulative)
            {
                ...
                {
                    float random =  UnityEngine.Random.Range((float)LopColeJar.instance.RailMint.cash_random[0], (float)LopColeJar.instance.RailMint.cash_random[1]);
    ...
    {
        return GetMulti(RewardType.Cash, AiryMintStrange.GetDouble(CTedium.Dy_InnovativeSeatSate), LopColeJar.instance.RailMint.gold_group);
    }

    public static double GetCashMulti()
    {
        return GetMulti(RewardType.Diamond, AiryMintStrange.GetDouble(CTedium.Dy_InnovativeParty), LopColeJar.instance.RailMint.cash_group);
    }
    /// <summary>
    /// 获取毫秒级时间戳（跨平台统一）
    /// </summary>
    ...
    public static LuckyObjData GetLuckyCardObjData()
    {
        ...
        List<RewardData> luckyDataListSource = LopColeJar.instance.RopeMint.lucky_card_data_list;
