...
public class CashOutPanel : SeedUIHouse
{
    ...
    private void Start()
    {
        CashOutManager.LawLaurasia()._CashOutPanel = this;
        CloseBtn.onClick.AddListener(() => { DriftUIBard(nameof(CashOutPanel)); });
        CashOutBtn.onClick.AddListener(() => { OnCashOutBtn(); });
        MoneyName = LopColeJar.instance.CashOut_Data.MoneyName;
        InfoText.text = LopColeJar.instance.CashOut_Data.Description;
        InfoPanel_OpenBtn.onClick.AddListener(() => OpenPanel(InfoPanel));
        InfoPanel_CloseBtn.onClick.AddListener(() => ClosePanel(InfoPanel));
        ContactBtn.GetComponent<Text>().text = LopColeJar.instance.SeedMaw;
        ContactBtn.onClick.AddListener(() =>
        {
            string url = LopColeJar.instance.SeedMaw;
        ...
        RecordPanel_OpenBtn.onClick.AddListener(() =>
        {
            ...
            CashOutManager.LawLaurasia().GetWithdrawRecord();
        });
        ...
        AccountText.text = CashOutManager.LawLaurasia().Account;
        Confirm_AccountText.text = CashOutManager.LawLaurasia().Account;
        if (string.IsNullOrEmpty(AccountText.text))
        {
            AccountText.text = "Account";
            Confirm_AccountText.text = "Account";
        }
        IDText.text = "UID:" + CashOutManager.LawLaurasia().Data.UserID;
        PolicyBtn.onClick.AddListener(() =>
        {
            string url = LopColeJar.instance.SeedMaw + "/privacy_policy.html";
#if UNITY_ANDROID || UNITY_EDITOR
            Application.OpenURL(url);
#elif UNITY_IOS
        openUrl(url);
        ...
        PolicyToggle.isOn = AiryMintStrange.GetBool("CashOut_PolicyAgree");
   ...
   RopeStrange.Instance.EnsueCylinder();
        UpdateUserInfo();
        if (ZJT_Manager.LawLaurasia().DisplayScale != Vector2.zero)
            transform.root.GetComponent<CanvasScaler>().referenceResolution = ZJT_Manager.LawLaurasia().DisplayScale;
    }
    ...
    public override void Hidding()
    {
        ...
        CashOutManager.LawLaurasia().WaitToSendEvent1304();
        if (ZJT_Manager.LawLaurasia().DisplayScale != Vector2.zero)
            transform.root.GetComponent<CanvasScaler>().referenceResolution = ZJT_Manager.LawLaurasia().DisplayScale;
        if (TendStand.Instance.TightSlope == 1)
            TendStand.Instance.Tight_2();
    }

    public void UpdateUserInfo() //拉取用户信息 显示加载界面
    {
        //StartLoadingAnim(UpdateUI_Loading);
        CashOutManager.LawLaurasia().UpdateUserInfo();
    }
    ...
    {
        if (CashOutManager.LawLaurasia().Data == null)
            return;
        if (string.IsNullOrEmpty(CashOutManager.LawLaurasia().Account))
        {
            OpenPanel(AccountPanel);
            return;
        }
    ...
    void OnConfimBtn() //确认修改账户
    {
        ...
        {
            JapanStrange.LawLaurasia().BeatJapan("Account cannot be empty");
            return;
        }
        if (!string.IsNullOrEmpty(AccountErrorText.text))
        {
            JapanStrange.LawLaurasia().BeatJapan(AccountErrorText.text);
            return;
        }

        CashOutManager.LawLaurasia().Account = AccountInput.text;
        AiryMintStrange.SetString("CashOut_Account", AccountInput.text);
    ...
    void OnSbmitBtn() //确认是否同意协议 同意后调用提现接口
    {
        ...
        {
            JapanStrange.LawLaurasia().BeatJapan("Please agree to the policy");
            return;
        }
        AiryMintStrange.SetBool("CashOut_PolicyAgree", true);
        StartLoadingAnim(CashOutBtn_Loading);
        StartLoadingAnim(SbmitBtn_Loading);
        CashOutManager.LawLaurasia().Withdraw();
    }
    ...
    public void UpdateMoney()
    {
        ...
        float MoneyEnd = CashOutManager.LawLaurasia().Money;
        DOTween.To(() => MoneyStart, x => MoneyText.text = x.ToString("F2"), MoneyEnd, 1f);
        CashText.text = CashOutManager.LawLaurasia().Data.Cash.ToString("F2");
        //进度条动画
        float MaxMoney = float.Parse(LopColeJar.instance.CashOut_Data.convert_goal, CultureInfo.CurrentCulture);
    ...
    public void MoneyToCashAnim()
    {
        ...
        float CashOutEnd = CashOutManager.LawLaurasia().Data.Cash;
    ...
    {
        if (CashOutManager.LawLaurasia().Data.TaskData != null && CashOutManager.LawLaurasia().Data.TaskData.Name != "Null")
        {
            // Cash余额任务 不走前端进度统计 使用后台Data.Cash数值
            if (CashOutManager.LawLaurasia().Data.TaskData.Name == "Cash" && CashOutManager.LawLaurasia().Data.Cash < CashOutManager.LawLaurasia().Data.TaskData.Target)
            {
                CashOutBtn.gameObject.SetActive(false);
                CashOutBtn_Task.SetActive(true);
                CashOutBtn_Task_Text.text = string.Format(CashOutManager.LawLaurasia().Data.TaskData.Description, CashOutManager.LawLaurasia().Data.Cash, CashOutManager.LawLaurasia().Data.TaskData.Target);
            }
            // 其他任务 前端统计任务完成进度 
            else if (CashOutManager.LawLaurasia().Data.TaskData.Name != "Cash" && CashOutManager.LawLaurasia().Data.TaskData.NowValue < CashOutManager.LawLaurasia().Data.TaskData.Target)
            {
                CashOutBtn.gameObject.SetActive(false);
                CashOutBtn_Task.SetActive(true);
                CashOutBtn_Task_Text.text = string.Format(CashOutManager.LawLaurasia().Data.TaskData.Description, CashOutManager.LawLaurasia().Data.TaskData.NowValue, CashOutManager.LawLaurasia().Data.TaskData.Target);
            }
            // 任务完成 显示提现按钮
            else
            {
    ...
    public void UpdateRecord(int Page = 0) //更新单页10个提现记录
    {
        ...
        if (CashOutManager.LawLaurasia().Data.Record == null || CashOutManager.LawLaurasia().Data.Record.Count == 0)
        {
            NoRecordTip.SetActive(true);
            return;
        }

        int RecordCount = CashOutManager.LawLaurasia().Data.Record.Count;
        ...
        List<WithdrawRecordItem> OnePageRecord = CashOutManager.LawLaurasia().Data.Record.GetRange(startIndex, itemsToShow);
    ...
    public void CloseLoading_Withdraw(bool IsCashOutSuccess = false)
    {
        ...
        if (IsCashOutSuccess)
        {
            JapanStrange.LawLaurasia().BeatJapan("Withdraw success");
            ClosePanel(ConfirmAccountPanel);
        }
    }
    ...
    {
        BabyLatinDating.LawLaurasia().TangLatin("1301", LeftTimeText.text, CashOutManager.LawLaurasia().Money.ToString());
    }
}
