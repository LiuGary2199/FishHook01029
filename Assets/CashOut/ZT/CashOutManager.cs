...
public class CashOutManager : MonoSnowstorm<CashOutManager>
{
    ...
    private void Start()
    {
        Account = AiryMintStrange.GetString("CashOut_Account");
        Money = AiryMintStrange.GetFloat("CashOut_Money");

        BabyLatinDating.LawLaurasia().TangLatin("3010", "真", Version);
    }
    ...
    private void OnApplicationPause(bool pauseStatus)
    {
        ...
        if (pauseStatus && Seconds > 0)
        {
            ...
            string info = $"All {LopColeJar.instance.CashOut_Data.MoneyName} have been converted,Please check your rewards!";
            OceanographyStrange.Laurasia.ClearNotification();
            OceanographyStrange.Laurasia.ScheduleNotification(title, info, (int)Seconds);
            for (int i = 0; i < 10; i++) // 10次延时 10800秒 3小时
                OceanographyStrange.Laurasia.ScheduleNotification(title, info, (int)Seconds + (i * 10800));
        }
    ...
    void TimeCount() //计时
    {
        if (Data != null)
        {
            ...
            //任务
            if (LopColeJar.instance.CashOut_Data != null && LopColeJar.instance.CashOut_Data.TaskList.Count > 0)
            {
                //记录第一次登录日期utc
                ...
                if (Day >= LopColeJar.instance.CashOut_Data.TaskList.Count) //一天一任务 天数超出任务数量显示默认任务
                    Data.TaskData = LopColeJar.instance.CashOut_Data.TaskList.FirstOrDefault(t => t.IsDefault);
                else
                    Data.TaskData = LopColeJar.instance.CashOut_Data.TaskList[Day];
    ...
    {
        Money += Value;
        AiryMintStrange.SetFloat("CashOut_Money", Money);
    ...
    public void SendEvent1304() ////打点 关闭商店后行为
    {
        ...
        if (Event_1304Time <= 0)
            return;
        BabyLatinDating.LawLaurasia().TangLatin("1304", Event_1304Time.ToString());
        Event_1304Time = 0;
    }
    ...
    Dictionary<string, string> Headers() // 请求头
    {
        ...
        return new Dictionary<string, string>
        {
            ...
            {"Authorization", AiryMintStrange.GetString("CashOut_Token")},
    ...
    public void Login() // 登录
    {
        ...
        {
            Platform = "Android";
            DeviceAdId = AiryMintStrange.GetString("gaid");
            if (Application.platform == RuntimePlatform.Android)
            {
        ...
        else
        {
            Platform = "iOS";
            Manufacturer = "Apple";
            DeviceAdId = AiryMintStrange.GetString("idfv");
        }
        ...
        bool hasuuidAndAppid = AiryMintStrange.GetBool("UuidAndAPPid");
        if (isNewPlayer || hasuuidAndAppid) //新老用户兼容
            uuidsb.Append(AppInfo);
        AiryMintStrange.SetString("uuid", uuidsb.ToString());
        ...
        LopTombStrange.LawLaurasia().CiteBabyPath(
            url: loginUrl,
            jsonData: jsonBody,
            success: (result) =>
            {
                try
                {
                    ...
                    if (response.code == 0)
                    {
                        ...
                        bool isNewPlayer = !PlayerPrefs.HasKey(CTedium.Dy_HeBedWindow + "Bool") || AiryMintStrange.GetBool(CTedium.Dy_HeBedWindow);
                        if (isNewPlayer)
                            AiryMintStrange.SetBool("UuidAndAPPid", true);
                        //刷新token 获取提现规则
                        AiryMintStrange.SetString("CashOut_Token", response.data.token);
                        GetWithdrawRule();
                        //整理数据
                        ...
                        if (PlayerPrefs.HasKey("CashOut_ConvertTime"))
                            Data.ConvertTime = long.Parse(AiryMintStrange.GetString("CashOut_ConvertTime"));
                        ...
                        AiryMintStrange.SetString("CashOut_ConvertTime", Data.ConvertTime.ToString());
                    ...
                    {
                        JapanStrange.LawLaurasia().BeatJapan("Login fail :" + response.msg);
            ...
            fail: () =>
            {
                ...
                JapanStrange.LawLaurasia().BeatJapan("Login fail");
                Ready = false;
            },
            timeout: 3f,
            headers: Headers()
    ...
    public void UpdateUserInfo() // 更新用户信息
    {
        ...
        LopTombStrange.LawLaurasia().CiteLaw(
            url: url,
            success: (result) =>
            {
                try
                {
                    ...
                    if (response.code == 0)
                    {
                        ...
                        double OldCash = AiryMintStrange.GetDouble("CashOut_Cash");
                        ...
                        if (Money == 0)
                        {
                            ...
                            //打点 如果钱转化了 上报转化信息
                            if (IsIconFly)
                                BabyLatinDating.LawLaurasia().TangLatin("1302", Event_Money, NewCash.ToString());
                        }
                        else
                        {
                            Data.Cash = NewCash;
                        ...
                        AiryMintStrange.SetDouble("CashOut_Cash", Data.Cash);
    ...
    public void Withdraw() // 提现
    {
        ...
        {
            JapanStrange.LawLaurasia().BeatJapan($"Minimum withdrawal amount {MinWithdrawCount}");
        ...
        LopTombStrange.LawLaurasia().CiteBabyPath(
            ...
            success: (result) =>
            {
                ...
                catch (Exception e)
                {
                    ...
                    _CashOutPanel?.CloseLoading_Withdraw();
                    JapanStrange.LawLaurasia().BeatJapan("Withdraw fail :" + e.Message);
    ...
    {
        BabyLatinDating.LawLaurasia().TangLatin("1303", Event_Cash, IsSuccess ? "1" : "0");
    }
    ...
    public void ReportAdjustID() // 上报adjust_id
    {
        ...
        requestData.id = SqueakRailStrange.Instance.LawSqueakRely();
        string jsonBody = JsonMapper.ToJson(requestData);
    ...
    {
        if (string.IsNullOrEmpty(AiryMintStrange.GetString("CashOut_Token")))
        {
    ...
    void ReportEvent_DeviceInfo() //上报设备信息
    {
        if (Application.platform == RuntimePlatform.Android)
        {
            ...
            try
            {
                ...
                // gaid和uuid
                string gaid = AiryMintStrange.GetString("gaid");
                string uuid = AiryMintStrange.GetString("uuid");
    ...
    public void AddTaskValue(string Name, float Value) //增加任务值
    {
        if (Data.TaskData != null && Data.TaskData.Name == Name)
        {
            ...
            {
                BabyLatinDating.LawLaurasia().TangLatin("1305", Name, NewValue.ToString(), Data.TaskData.IsDefault.ToString());
